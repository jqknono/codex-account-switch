...
| cloud 账号在授权设备上临近过期 | cloud saved account 的 token 剩余有效期 `< 120h`，当前设备等于有效 `autoRefreshDeviceName` | 插件自动刷新 token，并写回 synced cloud state。 |
| cloud 账号在非授权设备上临近过期 | cloud saved account 的 token 剩余有效期 `< 120h`，当前设备不是有效 `autoRefreshDeviceName` | 插件跳过自动 token 刷新；账号保持原 token，quota 刷新仍独立执行。 |
