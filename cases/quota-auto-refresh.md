...
| access token 剩余不超过 5 天 | 某个 saved account 的 access token 剩余有效期 `<= 5` 天，轮转 auto refresh 命中该账号 | 插件会在 quota 查询前先刷新 token；若允许自动 token 回写，则保存后的账号状态会写回最新 token。 |
| access token 已过期 | 某个 saved account 的 access token 已过期，轮转 auto refresh 命中该账号 | 插件会在 quota 查询前先刷新 token；若允许自动 token 回写，则保存后的账号状态不再停留在 `Token expired`。 |
