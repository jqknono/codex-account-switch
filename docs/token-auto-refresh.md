...
| Auto-refresh setting | `codex-account-switch.tokenAutoUpdate` | Defaults to `true`; when disabled, the timer skips token maintenance and still runs the independent quota step. |
...
| Setting | `codex-account-switch.tokenAutoUpdate` defaults to `true`; setting it to `false` disables automatic token refresh only. |
